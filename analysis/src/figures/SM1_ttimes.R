...
friction <- ggplot() +
  ...
  geom_point(
    data = ctar_metadata, aes(x = long, y = lat), color = "black",
    shape = 4,
    stroke = 2
  ) +
  scale_fill_distiller(
    palette = "Purples", na.value = "white", direction = 1,
    name = "Friction value \n (travel speed \n minutes per meter)",
...
pop <- ggplot() +
  geom_raster(data = pop_df, aes(x, y, fill = cut(wp_2015_1x1,
    breaks = pop_breaks,
    labels = pop_labs
  ))) +
  scale_fill_manual(
    values = pop_cols, na.translate = FALSE,
...
write_create(
  S1.1_inputs,
  here_safe("analysis/figs/supplementary/S1.1_inputs.jpeg"),
  ggsave_it,
  device = "jpeg",
  height = 8,
  width = 12
...
baseline %>%
  ...
  mutate(
    commune = gtruth_mora_raw$commune[match(commcode, gtruth_mora_raw$commcode)],
    type = "Friction surface",
    ttimes_reported = ttimes / 60,
    ttimes_est = gtruth_mora_raw$ttimes_est[match(
      commcode,
      gtruth_mora_raw$commcode
    )]
  ) -> baseline_mora
gtruth_mora_raw %>%
  ...
  bind_rows(select(
    baseline_mora, commcode, commune,
    ttimes_reported, ttimes_est, type
  )) -> gtruth_mora_comp

mora_raw <- ggplot(
  data = gtruth_mora_comp,
  aes(x = ttimes_reported + 0.01, y = reorder(commune, ttimes_est), fill = type)
) +
  geom_density_ridges(
    alpha = 0.75, color = "NA", aes(point_color = type),
  ...
  scale_discrete_manual(
    aesthetics = "point_color", values = c(NA, "darkred"),
  ...
  theme_minimal_hgrid() +
  labs(
    x = "Travel times (hrs)", y = "Communes ordered \n by travel times",
    tag = "A"
  ) +
...
IPM_raw <- ggplot() +
  ...
  geom_bezier2(data = bez_pts, aes(
    x = long, y = lat, group = group, color = ttimes,
    size = ifelse(index == 1, 0.15, 2)
  )) +
  geom_point(
    data = filter(bez_pts, index != 2), aes(x = long, y = lat, color = ttimes),
    alpha = 0.5
  ) +
...
write_create(
  S1.2_raw,
  here_safe("analysis/figs/supplementary/S1.2_raw.jpeg"),
  ggsave_it,
  width = 10,
  height = 7
)
...
gtruth %>%
  ...
  group_by(mode) %>%
  summarize(
    ttimes_est = mean(ttimes_est, na.rm = TRUE),
...
write_create(
  S1.3_modes,
  here_safe("analysis/figs/supplementary/S1.3_modes.jpeg"),
  ggsave_it,
  height = 6,
  width = 6
)
...
# facet label names
facet_labs <- c(
  "A) Distance (km)", "B) Travel times (hrs) \n unweighted",
...
write_create(
  S1.4_lms,
  here_safe("analysis/figs/supplementary/S1.4_lms.jpeg"),
  ggsave_it,
  device = "jpeg",
  height = 10,
  width = 12
...
points <- c(
  NA, summary(lm(mean ~ ttimes_un, data = filter(all, type == "point")))$r.squared,
  summary(lm(mean ~ log(distance), data = filter(all, type == "point")))$r.squared
)
communes <- c(
  summary(lm(mean ~ ttimes_wtd, data = filter(all, type == "commune_wtd")))$r.squared,
...
ttimes_district <- ggplot() +
  ...
  geom_point(
    data = ctar_metadata, aes(x = long, y = lat), color = "darkgrey", shape = 4,
    stroke = 2
  ) +
  theme_map() +
  labs(tag = "A")
...
write_create(
  S1.5_outputs,
  here_safe("analysis/figs/supplementary/S1.5_outputs.jpeg"),
  ggsave_it,
  device = "jpeg",
  height = 10,
  width = 12
