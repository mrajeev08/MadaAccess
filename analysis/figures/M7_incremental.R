...
# Vials
vials <- ggplot(data = natl_preds[scenario != max(scenario)], 
  ...
  geom_pointrange(data = natl_preds[scenario == max(scenario)],
                  aes(x = as.numeric(scenario), y = vials_mean,
...
# Burden
burden <- ggplot(data = natl_preds[scenario != max(scenario)], 
  ...
  geom_pointrange(data = natl_preds[scenario == max(scenario)],
                  aes(x = as.numeric(scenario), y = deaths_mean,
...
# Vials per death
vials_per_death <- ggplot(natl_preds[scenario != max(scenario)], 
  ...
  geom_pointrange(data = natl_preds[scenario == max(scenario)],
                  aes(x = as.numeric(scenario), y = vials_mean/averted_mean,
...
# Saving session info
out.session(path = "R/figures/M7_incremental.R", filename = "output/log_local.csv",
            start = start)
