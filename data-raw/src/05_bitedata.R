...
peripheral_comm_matches %>%
  ...
  select(
    names_tomatch, min_fixed, match, matchcode_comm,
    matchcode_data, commcode
  ) -> peripheral_comm_matches
peripheral %>%
...
peripheral %>%
  mutate(
    type = "new", date_reported = ymd(date_de_consultation),
    source = "peripheral"
  ) %>%
  select(
...
moramanga %>%
  mutate(
    type = if_else(Type.of.consultation == "Passage", "transfer",
      "new"
    ),
  ...
  select(
    date_reported, type, ctar, id_ctar, distcode, commcode, known_cat1,
    source
  ) -> moramanga_clean

# Output master data and summary stats -------------------------------------------------------------
national <- bind_rows(IPM_clean, peripheral_clean)
write_create(national, safe_path("data-raw/out/bitedata/national.csv"),
  write.csv,
  row.names = FALSE
)
write_create(moramanga_clean, safe_path("data-raw/out/bitedata/moramanga.csv"),
  write.csv,
  row.names = FALSE
)
...
write_create(moramanga_ttimes, safe_path("data-raw/out/bitedata/moramanga_ttimes.csv"),
  write.csv,
  row.names = FALSE
)

# Saving session info
out.session(path = "data-raw/src/05_bitedata.R",
