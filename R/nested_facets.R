##' ggplot helper for nested facet label from
##' https://github.com/teunbrand/ggnomics/blob/standalone-facet_nested/R/facet_nested.R
...
combine_nested_vars <- function(data, env = emptyenv(), vars = NULL, drop = TRUE) {
  ...
  values <- ggplot2:::compact(plyr::llply(data, ggplot2:::eval_facets,
    facets = vars,
    env = env
  ))
