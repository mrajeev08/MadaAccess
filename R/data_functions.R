...
ctar_to_exclude <- function(national, ctar_metadata, min_forms = 10) {
  ...
  national %>%
    filter(year(date_reported) > 2013, year(date_reported) < 2018, !is.na(distcode), 
           !is.na(id_ctar)) %>%
...
clean_natl <- function(national, mada_districts, mada_communes, ctar_metadata, 
  ...
  throughput %>%
    ...
    mutate(include_day = rle.days(no_patients, threshold = tput_thresh), 
  ...
  national %>%
    filter(distcode %in% mada_districts$distcode, 
  ...
  mada_districts %>%
    ...
    mutate(catch = as.numeric(droplevels(factor(catchment))), 
  ...
  mada_communes %>%
    filter(exclude_dist == 0) %>%
    mutate(catch = as.numeric(droplevels(factor(ctch_dist))),
           group = as.numeric(droplevels(factor(distcode)))) %>%
    arrange(group) %>%
