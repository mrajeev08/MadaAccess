...
mod_df %>%
  ...
  mutate(covar_preds = case_when(covar_name == "distance" & scale %in% c("Commune", 
         ...
         names_covar_preds = case_when(covar_name == "distance" & scale %in% c("Commune", 
                                                                         "Moramanga") ~ list(mada_communes$ADM3_PCODE),
                                 covar_name == "ttimes" & scale %in% c("Commune", 
                                                                       "Moramanga") ~ list(mada_communes$ADM3_PCODE),
                                 covar_name == "distance" & scale == "District" ~ list(mada_districts$distcode),
                                 covar_name == "ttimes" & scale == "District" ~ list(mada_districts$distcode)),
         names_preds = case_when(covar_name == "distance" & scale %in% c("Commune", 
                                                                               "Moramanga") ~ list(mada_communes$distcode),
                                       covar_name == "ttimes" & scale %in% c("Commune", 
                                                                             "Moramanga") ~ list(mada_communes$distcode),
                                       covar_name == "distance" & scale == "District" ~ list(mada_districts$distcode),
                                       covar_name == "ttimes" & scale == "District" ~ list(mada_districts$distcode)),
         pop_preds = case_when(covar_name == "distance" & scale %in% c("Commune", 
                                                                         "Moramanga") ~ list(mada_communes$pop),
                                 covar_name == "ttimes" & scale %in% c("Commune", 
                                                                       "Moramanga") ~ list(mada_communes$pop),
                                 covar_name == "distance" & scale == "District" ~ list(mada_districts$pop),
                                 covar_name == "ttimes" & scale == "District" ~ list(mada_districts$pop))) -> mod_df_preds
...
check %>%
  ...
  summarize(deaths_mean = sum(deaths_mean), deaths_upperCI = sum(deaths_upperCI), deaths_lowerCI = sum(deaths_lowerCI),
            averted_lowerCI = sum(averted_lowerCI), averted_upperCI = sum(averted_upperCI), averted_mean = sum(averted_mean)) -> total_preds

ggplot(data = total_preds, aes(x = covar_name, y = deaths_mean, color = scale)) + 
  geom_point() +
  geom_pointrange(aes(ymin = deaths_lowerCI, ymax = deaths_upperCI)) +
  scale_color_manual(values = c("darkred", "#cc7722", "#004b49"), name = "Scale")
...
preds_fixed <- foreach(i = iter(mod_df_preds, by = "row"), .combine = "rbind") %do% {
  ...
  burden_max <- get.burden.fixed(predicted_bites = preds$predicted, pop = preds$pop, rho_max = 0.98,
                             p_rabid = 0.2, HDR = 5, dog_rabies_inc = 0.01, human_exp_rate = 0.39, 
                             prob_death = 0.16)
...
ggplot() +
  geom_linerange(data = check, aes(x = names_bites, ymin = deaths_min, 
                                   ymax = deaths_max)) +
  facet_grid(scale ~ covar_name, scales = "free", 
             labeller = labeller(pop_predict = as_labeller(mod_names)), drop = TRUE) +
  expand_limits(y = 0) +
