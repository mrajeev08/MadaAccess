...
out.session <- function(logfile = "log.csv", start = NULL, ncores = 1) {
  ...
  if(is.null(path)) {
    ...
    toappend <- data.table::data.table(ran = timestamp(),
...
this_file <- function() {
  ...
  sysf <- trimws(sub(here::here(), "", path.expand(sys.frames()[[1]]$ofile)), whitespace = '/')

  # Rscript
  if (length(match) > 0) return(match)

  # 'source'd via R console
  if(!is.null(sysf)) return(sysf)

  if(is.null(sysf) & length(match) == 0) return(NULL)
...
setup_cl <- function(slurm = Sys.getenv('SLURM_JOB_ID') != "",
  ...
  if(!slurm) {
    ...
    if (!is.na(args[1])) {
      ...
      make_cl <<- function() { parallel::makeCluster(parallel::detectCores() - 1) }
  ...
  return(list(start = start, ncores = ncores, slurm = slurm,
              logfile = logfile))
