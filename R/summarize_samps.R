...
summarize_samps <- function(parent_dir = "analysis/out/mods/samps/") {
  ...
  file_dt <- data.table(
    filenames = files,
    pop_predict = case_when(
      grepl("flatPop", files, fixed = TRUE) ~ "flatPop",
      grepl("addPop", files, fixed = TRUE) ~ "addPop",
      grepl("onlyPop", files, fixed = TRUE) ~ "onlyPop"
    ),
    scale = case_when(
      grepl("Commune", files, fixed = TRUE) ~ "Commune",
      grepl("District", files, fixed = TRUE) ~ "District"
    ),
    intercept = case_when(
      grepl("fixed", files, fixed = TRUE) ~ "fixed",
      grepl("random", files, fixed = TRUE) ~ "random"
    ),
    data_source = case_when(
      grepl("Moramanga", parent_dir,
        fixed = TRUE
      ) ~ "Moramanga",
