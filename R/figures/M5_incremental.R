...
admin_preds <- fread("output/preds/burden/burden_complete.gz")
...
natl_preds_all$scenario[natl_preds_all$scenario == max(natl_preds_all$scenario)] <- 800
...
natl_preds$scenario[natl_preds$scenario == max(natl_preds$scenario)] <- 800
ggplot(data = filter(natl_preds, scenario != 800), 
  ...
  geom_point(data = filter(natl_preds, scenario == 600), 
             aes(x = scenario, y = deaths_mean, color = scale)) +
  geom_pointrange(data = filter(natl_preds, scenario == 600),
