...
neg_rel <- ggplot(data = filter(inc_scaled_rel, scaling == "neg")) +
  ...
  scale_color_manual(values = c("#F2300F", "#35274A"), name = "Scale", guide = "none") +
...
neg_map <- ggplot(data = filter(gg_all, scaling == "neg")) +
  geom_polygon(aes(x = long, y = lat, group = group, alpha = inc_scaled*1e5,
                   fill = scale),
               color = NA) +
  scale_fill_manual(values = c("#F2300F", "#35274A"), guide = "none") +
  scale_alpha_continuous(range = c(0.15, 1), breaks = c(0, 15, 25, 50, 75, 110), 
                         name = "Exposures \n per 100k", 
                         guide = guide_legend(override.aes = list(fill = "#F2300F"))) +
...
pos_rel <- ggplot(data = filter(inc_scaled_rel, scaling == "pos")) +
  geom_line(aes(x = pop, y = inc_scaled*1e5, color = scale), size = 2, alpha = 0.75) +
  scale_x_continuous(trans = "log", breaks = c(1000, 1e4, 1e5, 1e6)) +
  scale_color_manual(values = c("#F2300F", "#35274A"), name = "Scale") +
  labs(x = "Population", y = "Exposure incidence \n per 100k", tag = "B") +
  theme_minimal_hgrid() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
base_scaling_A <- ggplot(data = base_se, 
                      aes(x = ttimes, y = deaths_mean/pop*1e5, color = scale, fill = scale,
                          group = interaction(scale, scaling))) +
  geom_line(data = filter(base_se, scaling == "base"), alpha = 0.75) +
  geom_pointrange(data = filter(base_se, scaling != "base"),
             aes(ymin = deaths_upper/pop*1e5, ymax = deaths_lower/pop*1e5), 
             alpha = 0.25) +
  geom_ribbon(data = filter(base_se, scaling == "base"),
              aes(ymax = deaths_upper/pop*1e5, ymin = deaths_lower/pop*1e5), 
              color = NA, alpha = 0.25) + 
  facet_grid(scaling ~ scale, scales = "free_x") + # dummy to get legend right
...
# Add ARMC level ----------------------------------
add_scaled %>%
  group_by(scale, scenario, scaling) %>%
  summarize_at(vars(bites_mean:averted_lower), sum) %>%
  pivot_longer(bites_mean:averted_lower) %>%
  filter(grepl("deaths", name)) -> add_summ
...
add_se %>%
  group_by(scaling, scale, name) %>%
  arrange(scenario) %>%
  mutate(value = value/value[1]) %>%
  pivot_wider(id_cols = c(scale, scenario, scaling), 
              names_from = name) -> add_props
add_se %>%
  pivot_wider(id_cols = c(scale, scenario, scaling), 
              names_from = name) -> add_totals

max_added <- sort(unique(add_totals$scenario), decreasing = TRUE)[2]
max_total <- max_added + 200
add_totals$scenario[add_totals$scenario == max(add_totals$scenario)] <- max_total
add_totals$scaling <- factor(add_totals$scaling, levels = c("neg", "base", "pos"))

add_scaling_B <- ggplot(data = filter(add_totals, scenario != max_total), 
                         aes(x = scenario, y = deaths_mean, group = interaction(scale, scaling), 
                             color = scale, fill = scale)) +
  geom_line(alpha = 0.75) +
  geom_ribbon(data = filter(add_totals, scenario != max_total),
              aes(ymax = deaths_upper, ymin = deaths_lower), color = NA, alpha = 0.25) + 
  geom_pointrange(data = filter(add_totals, scenario == max_total),
  ...
  facet_grid(scaling ~scale, scales = "free_x", labeller = labeller(scaling = scaling_labs)) + 
  scale_color_manual(values = model_cols, labels = scale_labs, aesthetics = c("color", "fill"),
                     name = "Model scale (& data)", guide = "none") +
  theme_minimal_grid() +
  labs(x = "# Additional ARMC", y = "Total deaths (national)", tag = "B") +
  theme()

# fig S6.5
S6.5_scaling_se <- (base_scaling_A | add_scaling_B) + plot_layout(guides = "collect")
