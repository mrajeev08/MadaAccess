...
## check just with 10 
check <- add.armc(current_ARMC = gps_locs, candidate_ARMC = csbs[seq(1, 1000, by = 100), ], prop_pop, 
                  threshold, delta_tt_min = 1e-5,
                  steps = 10, base_prop, 
                  friction = friction_masked, shapefile = mada_district, 
                  filename_trans = "output/trans_gc_masked.rds")

write.csv(check, "output/check_incremental.csv")
