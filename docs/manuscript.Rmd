...
For the national data, some clinics did not report any data, and for others there were substantial periods, months to even a whole year, when no patient forms were submitted. To correct for this, we estimated clinic level undersubmission as the proportion of days on which forms were submitted in a given year, excluding any periods for which there were no forms submitted for 15 consecutive days. We then divided the total number of bites reported to each district at each clinic in a given year by the undersubmission rate for that clinic in that year. Due to the variation in submission rates across time, we chose to take the average of the annual bite incidence estimates. The data from the Moramanga district were completely reported across the 31 month period.
...
Since we're assuming for poisson errors: we tested for overdispersion. 

Finally, to check how correcting for incomplete submission of forms affected our modeling results, we fitted these same models to uncorrected data.
...
include_graphics(here("figs/main/M2.jpeg"))
```
...
## Estimating travel times to the closest ARMC
```{r ttime stats}
...
include_graphics(here("figs/main/M3_ttimes.jpeg"))
```
...
Clinic submission of forms was highly variable over the period covered by the dataset (Figure S2.1). However, we found that corrected estimates of reproted bites generated predictions of vial demand close to what was provisioned over the same time period ([Figure S2.2A and B](#figS2.2)). We excluded any districts in a catchment of a clinic which submitted less than 10 forms (excluded `r nrow(ctar_metadata) - max(district_bites$catch)` catchments, corresponding to `r length(mada_districts) - nrow(district_bites)` districts, in grey in Fig 3A). After additionally excluding any year with less than 25% of forms submitted, our final dataset consisted of estimates of average bite incidence for `r nrow(district_bites)` out for `r length(mada_districts)` districts total (Fig 4A), and `r nrow(mora_bites)` communes within the catchment of the Moramanga District (Fig 4B). 
...
include_graphics(here("figs/main/M4.jpeg"))
```
...
include_graphics(here("figs/main/M5.jpeg"))
```
...
## Broader context
```{r}
burden_preds <- fread(here("output/preds/complete/burden_filled.csv"))
## Grouped to district
...
CATCHMENT
Most districts and communes a single clinic served the majority of the population by this metric (Fig S2.1B). In addition, our data show that the majority of bite patients do report to the closest clinic as assigned by our travel time metric (Fig S2.1C). 
...
include_graphics(here("figs/supplementary/S3.2.jpeg"))
```
...
include_graphics(here("figs/supplementary/S3.3.jpeg"))
```
